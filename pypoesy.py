...
import pytxt,pystats,cPickle,pymongo

# models the tml files in poet and poem folders
#poetfolder='/Volumes/LitLab/Lab/Projects/biblio/poets'
#poemfolder='/Volumes/LitLab/Lab/Projects/biblio/poems'
poetfolder='/Dropbox/LITLAB/poetry/CORPUS/poets'
poemfolder='/Volumes/LilaData1/biblio/poems' if False else '/Users/ryan/poemsample'
#poemshelf='/Volumes/LilaServerOS/LilaDataTmp/poemshelf'
poemshelf='/Users/ryan/Desktop/tmp/poemshelf'

folder_concordance_poet='/Lab/Processing/tsv/tokenfreqs_poets'
RHYME_SCHEMES=[d for d in pytxt.xls2ld('/Dropbox/LITLAB/poetry/rhyme/schemes.xls') if not 'sonnet' in d['form'].lower()]

MONGO=pymongo.Connection()
POEMDB=MONGO.poetry.poems
POETDB=MONGO.poetry.poets



def etym_year_ted(fn='/Dropbox/LITLAB/1750-1850/ted_jdh/Etymologies.txt'):
	yeard={}
	for ln in codecs.open(fn,encoding='utf-8'):
		word,year,count=ln.split('\t')
		if int(year)<100:
			yeard[word]='FunctionWord'
		else:
			yeard[word]='Pre-1150' if int(year)<1150 else 'Post-1150'
	return yeard

word2year=etym_year_ted()


#spelling_d=dict((d['variant'],d['standard']) for d in pytxt.tsv2ld('/Dropbox/LITLAB/TOOLS/spellings/variants.txt',header=['variant','standard','']))

def dir2tml(dirpath):
	for fn in os.listdir(dirpath):
		if not fn.endswith('.txt'): continue
		txt=open(os.path.join(dirpath,fn)).read()
		txt2tml(txt,title=fn.replace('.txt',''))

def wordsworth2tml(dirpath='/Dropbox/LITLAB/poetry/Wordsworth1'):
	l=[]
	for fn in os.listdir(dirpath):
		if not fn.endswith('.txt'): continue
		txt=open(os.path.join(dirpath,fn)).read().replace('\r\n','\n').replace('\r','\n')
		lines=txt.split('\n')
		lines=[lx.strip() for lx in lines if not lx.strip() or lx.strip().split()[0].isdigit()]
		lines=[' '.join([word for word in lx.split() if not word.isdigit()]) if lx else lx for lx in lines]
		newtxt='\n'.join(lines)
		l+=[txt2tml(newtxt,title=fn.replace('.txt',''))]
	pytxt.write2('wordsworth_tmls.txt',l)



def txt2tml(txt,title="Unknown",author="Unknown"):
	tmlfn=pytxt.hash(txt)
	domstr="""
	<head>
		<author><lname>{2}</lname></author>
		<title id="{0}">{1}</title>
		<meta></meta>
	</head>

	<body></body>
	""".format(tmlfn,title,author)
	import bs4,codecs
	dom=bs4.BeautifulSoup(domstr)
	linenum=0
	stanzanum=0
	for stanza in txt.split('\n\n'):
		stanza=stanza.strip()
		stanzanum+=1
		stobj=dom.new_tag('div',type='stanza',n=stanzanum)
		dom.find('body').append(stobj)
		for line in stanza.split('\n'):
			line=line.strip()
			if not line: continue
			linenum+=1
			lobj=dom.new_tag('div',type="line",n=linenum)
			lobj.string=line
			stobj.append(lobj)

	ostr=dom.prettify()
	ofnfn=os.path.join(poemfolder,'X_'+tmlfn+'.tml')
...
def wcw(): return poet(2290)

def to_wordfreqs():
	import re,pymongo
	db=pymongo.Connection().wordfreqs.ngram1
	db.remove({'language':'English', 'genre':'Poetry'})
	poetlist=poets()
	numpoets=len(poetlist)
	for poet_i,poet in enumerate(poetlist):
		print poet_i,numpoets,'...'
		poetdx=dict((k,poet.data.get(k)) for k in ['dob', 'sex', 'lname', 'periods', 'fname', 'nationality', 'movements'])
		try:
			poetdx['year']=int(poetdx['dob'])+30
		except (TypeError,ValueError) as e:
			print e
			poetdx['year']=''
		poetdx['author']='lname'
		poetdx['title']=poetdx['author']
		poetdx['genre']='Poetry'
		poetdx['language']='English'
		...
		sumfreq=sum(words.values())
		poetdx['num_words_in_title']=sumfreq
		for word,raw in words.items():
			word_dx=dict(poetdx.items())
			word_dx['word']=word
			word_dx['raw']=raw
			word_dx['freq']=raw/sumfreq
			db.insert(word_dx)
...
def bv(linelim=100):
	...
	ww=Author(2325)
	byron=Author(400)
	import prosodic as p,random
	poets=[byron]
	ld=[]
	def num_vowells(string):
		vowels=['a','e','i','o','u']
		mouth='closed'
		numv=0
		for word in string.lower().split():
			mouth='closed'
			for letter in word:
				if mouth=='closed' and letter in vowels:
					mouth='open'
					numv+=1
				elif mouth=='open' and letter in vowels:
					pass
				else:
					mouth='closed'
		return numv
	...
	old=[]
	for poet in poets:
		#poems=random.sample(poet.poems(),25)
		#poems=[poem for poem in poet.poems() if poem.id=='Z200294020']
		for poem in poems:
			poem.gen()
			lines=poem.lines
			linenum=0
			...
			poemld2=[]
			for line in lines:
				line=line.replace('&indent;','').split('\n')[0]
				print [line]
				if pytxt.ascii(line)!=line: continue
				
				if not len(line.strip().split()): continue
				Line=p.Line(line)
				if Line.broken: continue
				linenum+=1
				
				dx={'num_sylls':len(Line.syllables()), 'num_words':len(Line.words()), 'num_vowells':num_vowells(line)}
				for k in dx:
					dx2={'measure':k, 'value':dx[k]}
					poemld2+=[dx2]
				print dx['num_sylls']
				poemld+=[dx]
			
			poemd={'poet':poet.lname, 'poem':poem.title, 'num_lines':len(lines), 'num_lines_understood':linenum}
			for k in ['num_sylls','num_words','num_vowells']:
				poemd['median_'+k]=pystats.median([dx[k] for dx in poemld])
			
			import rpyd2
			r=rpyd2.RpyD2(poemld2)
			title='\n'.join('{0}: {1}'.format(k,v) for k,v in sorted(poemd.items()))
			
			r.plot(fn=poet.lname+'.'+str(int(poemd['median_num_sylls'])).zfill(2)+'.'+poem.title[:25], x='value',density=True, col='measure',group='measure', title=title, density_metric='..count..')
	...
	#pytxt.write2('blankverse2.txt',old)

def combine_samples():
	periods=[
	 u'1500-1603_Tudor_Period',						# 0
	 u'1603-1660_Jacobean_and_Caroline_Period',		# 1
	 u'1660-1700_Restoration_Period',				# 2
	 u'1700-1745_Augustan_Period',					# 3
	 u'1745-1785_Age_of_Sensibility',				# 4
	 u'1780-1837_Romantic_Period',					# 5
	 u'1837-1901_Victorian_Period',					# 6
	 u'1900-1999_Twentieth_Century']				# 7

	canon2period={
		'Milton':periods[2],
		'Blake':periods[5],
		'Wordsworth':periods[5],
		'Yeats':periods[7],
		'Spenser':periods[0],
		'Whitman':periods[6],
		'Shakespeare':periods[0],
		'Dickinson':periods[6],
		'Keats':periods[5],
		'Byron':periods[5],
		'Shelley':periods[5],
	...
	sample1=MONGO.poetry.sample_periods
	old=list(sample1.find())

	sample2=MONGO.poetry.sample_canon
	id2poet={}
	for d in sample2.find():
		d['sample_period']=canon2period.get(d['author_lname'])
		d['name']=d['author_lname']
		old+=[d]
	...
	for d in old:
		try:
			y2=float(d['meter_perc_lines_fourthpos_s'])
			x2=float(d['meter_mpos_ww'])
		except (ValueError,KeyError) as e:
			continue
		dist=distance((x,y),(x2,y2))
		d['HOOD_DIST']=dist

	pytxt.write2('COMBINED_SAMPLE.txt', old)



def make_canon_sample():
	import random
	odb=MONGO.poetry.sample_canon
	odb.remove()
	odb.ensure_index('id')
	...
	for name,id in canon.items():
		poet=Author(id)
		poet.gen()
		poems = poet.poems()
		poems = random.sample(poems,50)
		
		for poem in poems:
			poemd=POEMDB.find_one({'id':poem.id})
			print odb.insert(poemd)

def parse_sample(sample='sample_canon'):
	global poemfolder
	poemfolder='/Dropbox/LITLAB/poetry/CORPUS/'+sample
	odb=getattr(MONGO.poetry,sample)

	def parse_poem(args):
		(i,dx)=args
		idx=dx['id']
		print ">>>> STARTING:"*5,i,idx
		poem=Poem(idx)
		poem.gen()
		poem.parse(100)
		for k,v in poem.statd.items(): dx[k]=v
		print odb.save(dx)
		print ">>>> DONE:"*5,i,idx

	todo=[]
	for i,dx in enumerate(odb.find()):
		if 'syll_scheme_repr' in dx: continue
		todo+=[(i,dx)]

	pytxt.crunch(todo, parse_poem)
...
def authors(ext='.tml'):
	return [Author(x.replace(ext,'')) for x in os.listdir(poetfolder) if x.endswith(ext)]

def poetld(fnfn='/Dropbox/LITLAB/poetry/CORPUS/poets.txt',mongo=True):
	#if not os.path.exists(fnfn):
	if mongo:
		import pymongo
		odb=pymongo.Connection().poetry.poets
		odb.remove()
		odb.ensure_index('id')
		odb.ensure_index('poem_ids')
		odb.ensure_index('name')

	if True:
		Poets=poets()
		old=[]
		for poet in Poets:
			print poet.data.keys()
			poetd={}
			for k,v in poet.data.items():
				if type(v) in [str,unicode]:
					poetd[k]=v

			try:
				poetd['num_cites']=int(poet.data['count']['type']['raw'])
			except KeyError:
				poetd['num_cites']=0
			try:
				poetd['num_poems']=len(poet.data['poem']['id'])
				poetd['poem_ids']=sorted(poet.data['poem']['id'].keys())
			except KeyError:
				poetd['poem_ids']=[]
				poetd['num_poems']=0

			poetd['name']=poetd['lname'] if not 'fname' in poetd else poetd['fname']+' '+poetd['lname']
			poetd['id']=poet.id
			if mongo: print odb.insert(poetd)
			old+=[poetd]
		pytxt.write2(fnfn,old)
		return old

	return pytxt.tsv2ld(fnfn)



def poets(fnfn='/Dropbox/LITLAB/poetry/CORPUS/poets.pickle',gen=True):
	import cPickle
	if not os.path.exists(fnfn):
		#poets=[Author(os.path.basename(x.replace('.tsv',''))) for x in os.listdir(folder_concordance_poet) if x.endswith('.tsv')]
		poets=[Author(fn.split('.')[0]) for fn in os.listdir(poetfolder)]
		if gen:
			nummax=len(poets)
			for i,poet in enumerate(poets):
				...
				poet.gen()
				print
			cPickle.dump(poets, open(fnfn,'wb'))
			print ">> saved:",fnfn
	else:
		poets=cPickle.load(open(fnfn))
	
	return poets

def poet(id,savedir='/Lab/Lila/data/poets/',gen=True,gen2=True):
	import cPickle
	fnfn=os.path.join(savedir,id)+'.pickle'
	if not os.path.exists(fnfn):
		poet=Author(int(id))
		if gen: poet.gen()
		if gen2:
			...
			for i,poem in enumerate(poems):
				print i, numpoems,poem
				poem.gen()
		cPickle.dump(poet, open(fnfn,'wb'))
		print ">> saved:",fnfn
	else:
		print ">> loading:",fnfn
		poet=cPickle.load(open(fnfn))
	return poet

def poetz(takeroot=False):
	pf='/Lab/Processing/zodb/poets.fs'
	import pyzodb
	root=pyzodb.genfdb(pf)
	if (not 'poets' in root):
		root['poets']={}
		for poet in poets():
			poet.gen()
			poet.loadConcordance()
			root['poets'][poet.id]=poet
		pyzodb.commit()
		
	if takeroot:
		return root
	else:
		return sorted(root['poets']).values()
...
class Poem(Author):
	...
	def statd(self):
		...
		syll_stresses=[]
		int_num_sylls=0
		for line in self.prosodic.values():
			line_stress=''
			for word in line.words():
				#if word.origin=='openmary': continue	# CONTROVERSIAL, but openmary's syllabs are not always accurate
				stress=word.stress.replace('P','1').replace('S','0').replace('U','0')
				#stress=word.stress
				weight=word.weight
				word_stressweight='_'.join(s+w for s,w in zip(stress,weight))
				all_words+=[word.token.lower()]

				for syll in word.syllables():
					int_num_sylls+=1
					shape=syll.children[0].getShape()
					syll_weight='Heavy' if syll.str_weight().startswith('H') else 'Light'
					syll_stress='Stressed' if syll.feature('prom.stress') else 'Unstressed'
					stressweight=syll_stress+'_'+syll_weight
					syll_shapes+=[shape]
					syll_weights+=[syll_weight]
					syll_stresses+=[syll_stress]
					syll_stressweights+=[stressweight]

					

				line_stress+=stress
				num_sylls+=[len(stress)]
				stress_types+=[stress]
				word_weights+=[weight]
				word_stressweights+=[word_stressweight]
				if word.token in word2year: etyms+=[word2year[word.token]]
			for seq in pytxt.bigrams(line_stress):
				stress_seqs+=[''.join(seq)]


		etc={'numsyll':num_sylls,
			'wordstress':stress_types,
			'wordweight':word_weights,
			'word_stressweight':word_stressweights,
			'etym':etyms, 
			'stress_seq':stress_seqs,
			'syll_shape':syll_shapes, 
			'syll_weight':syll_weights,
			'syll_stress':syll_stresses,
			'syll_stressweight':syll_stressweights
			}
		for ek,el in etc.items():
			for xk,xv in sorted(pytxt.toks2freq(el,tfy=True).items(), key=lambda _lt: -_lt[1])[:5]:
				dx['freq_'+ek+'_'+str(xk)]=xv

		dx['type_token_ratio']=float(len(set(all_words)))/len(all_words)
		dx['num_words']=len(all_words)
		dx['num_words_unique']=len(set(all_words))
		dx['num_sylls']=int_num_sylls
